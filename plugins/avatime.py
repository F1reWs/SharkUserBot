...
async def avatime(client, message):
  await message.edit_text("Creating picture...")
  while True:
  ...
  while True:
    ...
    font = ImageFont.truetype("Machine BT.ttf", 250)
    draw = ImageDraw.Draw(image)
    draw.text((500, 500), f"{Hour}:{Minute}", anchor = "mm", fill = "green", font = font)
    image.save("avatime.png")
    await client.set_profile_photo(photo="avatime.png")
    await asyncio.sleep(60)
    photos = [p async for p in client.get_chat_photos("me")]
    await client.delete_profile_photos([p.file_id for p in photos[1:]])

module_list['Avatime'] = f'{prefix}avatime'
